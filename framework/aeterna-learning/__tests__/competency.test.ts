...
function runMultiCompetencyTests() {
  ...
  // TEST 3: 3 ejercicios de razonamiento solo provocan brechas críticas para lo verdaderamente ausente (ej. TRANSFER)
  {
    const mock = createMockParsedArticle({
      exercises: [
        { id: '1', type: 'AeternaExercise', layerId: 'inicio', hasHint: true, hasXP: true, hasSolution: true, questionText: 'Estime de forma rápida de Fermi', detectedCognitiveLevels: ['reasoning_estimate'] },
        { id: '2', type: 'AeternaExercise', layerId: 'intermedio', hasHint: true, hasXP: true, hasSolution: true, questionText: 'Proponga una simplificación de modelo físico', detectedCognitiveLevels: ['reasoning_model'] },
        { id: '3', type: 'AeternaDecisionBox', layerId: 'avanzado', hasHint: true, hasXP: true, hasSolution: true, questionText: '¿Por qué la estimación funciona?', detectedCognitiveLevels: ['reasoning_justify'] }
      ]
    });
    const result = analyzeCompetencies(mock, 'methodological');
    assert(
  ...
  // TEST 4: Una competencia no relevante (ej. CALCULATE) no genera recomendación
  {
    const mock = createMockParsedArticle({
      exercises: [
        { id: '1', type: 'AeternaExercise', layerId: 'inicio', hasHint: true, hasXP: true, hasSolution: true, questionText: 'P1' }
      ]
    });
    const result = analyzeCompetencies(mock, 'methodological');
  ...
  // TEST 6: Un ejercicio de justificación cubre parcialmente EXPLAIN, ANALYZE y EVALUATE
  {
    const mock = createMockParsedArticle({
      exercises: [
        { id: 'ex3', type: 'AeternaDecisionBox', layerId: 'avanzado', hasHint: true, hasXP: true, hasSolution: true, questionText: 'Justifique por qué la simplificación es razonable', detectedCognitiveLevels: ['reasoning_justify'] }
      ]
    });
    const result = analyzeCompetencies(mock, 'methodological');
  ...
  // TEST 7: Solo generar FAIL cuando no exista evidencia tras analizar todas las actividades
  {
    const mock = createMockParsedArticle({
      exercises: [
        { id: '1', type: 'AeternaExercise', layerId: 'inicio', hasHint: true, hasXP: true, hasSolution: true, questionText: 'P1' },
        { id: '2', type: 'AeternaExercise', layerId: 'intermedio', hasHint: true, hasXP: true, hasSolution: true, questionText: 'P2' },
        { id: '3', type: 'AeternaExercise', layerId: 'avanzado', hasHint: true, hasXP: true, hasSolution: true, questionText: 'P3' }
      ]
    });
    const result = analyzeCompetencies(mock, 'methodological');

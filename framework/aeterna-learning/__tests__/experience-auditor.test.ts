...
function runExperienceAuditorTests() {
  ...
  // 2. Artículo conceptual -> Evalúa experiencias conceptuales (EXPLAIN, COMPARE) sin exigir cálculo
  {
    const mock = createMockArticle({
      ...
      exercises: [
        { id: '1', type: 'AeternaExercise', layerId: 'inicio', hasHint: true, hasXP: true, hasSolution: true, questionText: '¿Cuál es la diferencia entre masa inercial y gravitatoria?', detectedCognitiveLevels: ['conceptual_compare'] }
      ]
    });
    const report = evaluateScoring(mock, profile);
    assert(
  ...
  // 3. Artículo cuantitativo / problem_solving -> Exige resolución de problemas y cálculo autónomo
  {
    const mock = createMockArticle({
      ...
      exercises: [
        { id: '1', type: 'AeternaExercise', layerId: 'intermedio', hasHint: true, hasXP: true, hasSolution: true, questionText: 'Calcule la aceleración del bloque', detectedCognitiveLevels: ['procedural_apply'] }
      ]
    });
    const report = evaluateScoring(mock, profile);
    assert(
  ...
  // 5. Artículo con poca necesidad de interactividad -> No penaliza seriamente por falta de interactivo
  {
    const mock = createMockArticle({
      ...
      exercises: [
        { id: '1', type: 'AeternaExercise', layerId: 'inicio', hasHint: true, hasXP: true, hasSolution: true, questionText: 'Defina metro', detectedCognitiveLevels: ['conceptual_explain'] }
      ],
      interactives: []
    });
    const report = evaluateScoring(mock, profile);

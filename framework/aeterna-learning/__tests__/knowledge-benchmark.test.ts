...
function runKnowledgeBenchmarkTests() {
  ...
  const mockParsed: ParsedArticleStructure = {
    ...
    layers: [],
    exercises: [
      { id: '1', type: 'AeternaDecisionBox', layerId: 'intermedio', hasHint: true, hasXP: true, hasSolution: true, questionText: 'Decida' }
    ],
    interactives: [
      { type: 'AeternaDecisionBox', name: 'DecisionBox', line: 10, interactiveValue: { score: 4, criteria: { manipulateVariables: true, observeDynamicChanges: true, experiment: true, immediateFeedback: true, visualizeHardConcept: true }, interpretation: 'recomendado' } }
    ],

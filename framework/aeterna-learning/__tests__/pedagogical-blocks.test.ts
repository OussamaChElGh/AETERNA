...
function runPedagogicalBlocksTests() {
  ...
  // Test 4: MiniChallenge != AeternaExercise
  const mini = PEDAGOGICAL_BLOCKS_REGISTRY['mini-challenge'];
  const ex = ACTIVITY_CAPABILITY_REGISTRY['AeternaExercise'];
  assert(
    mini.blockType !== ex.componentType &&
    mini.family === 'PEDAGOGICAL_CONTENT_BLOCK' &&
    ex.description !== mini.description,
    '4. MiniChallenge mantiene identidad propia distinta de AeternaExercise'
  );
  ...
  // Test 7: Transfer != AeternaExercise
  ...
  assert(
    ...
    '7. Transfer mantiene identidad propia distinta de AeternaExercise'
  );
  ...
  // Test 10: AeternaSystem provides procedural reasoning
  ...
  assert(
    ...
    '10. AeternaSystem proporciona razonamiento crítico y metodológico'
  );

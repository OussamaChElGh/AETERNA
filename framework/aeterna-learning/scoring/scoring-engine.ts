...
function frameworkConfigPath(...segments: string[]): string {
  return path.join(process.cwd(), 'framework', 'aeterna-learning', 'config', ...segments);
}
...
export function evaluateScoring(
  ...
  // NEW: Aeterna Experience Result
  const aeternaBlocksCount = (parsed.rawBody.match(/<(?:PedagogicalContentBlock|Connect|HiddenAssumption|Transfer)/g) || []).length;
  const aeternaExperienceResult: AeternaExperienceResult = {
    connectionsScore: parsed.rawBody.includes('Connect') ? 90 : 60,
    experienceScore: aeternaBlocksCount >= 3 ? 95 : 70,
    overallScore: aeternaBlocksCount >= 3 ? 92 : 65,
    pedagogicalBlocksCount: aeternaBlocksCount,
    meaningfulIntegrationScore: aeternaBlocksCount > 0 ? 90 : 50,
    reflectionMomentsCount: (parsed.rawBody.match(/<AeternaDecisionBox/g) || []).length
  };
  ...
  // Aeterna experience: pedagogical blocks + cuaderno + planned images.
  ...
  return {
    ...
    scores: {
      ...
      experienciaAeterna: experienciaScore,

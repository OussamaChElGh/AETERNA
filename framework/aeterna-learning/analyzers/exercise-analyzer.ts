...
export function analyzeExercises(
  ...
  const aeternaExerciseRegex = /```aeterna-exercise([\s\S]*?)```/g;
  let match: RegExpExecArray | null;

  while ((match = aeternaExerciseRegex.exec(rawBody)) !== null) {
  ...
  const aeternaDecisionRegex = /```aeterna-decision([\s\S]*?)```/g;
  while ((match = aeternaDecisionRegex.exec(rawBody)) !== null) {
    const blockContent = match[1] || '';
    const matchIndex = match.index;
    const lineNumber = rawBody.substring(0, matchIndex).split('\n').length;
    const layerId = getLayerForLine(lineNumber);
  ...
  while ((match = fencedInteractiveRegex.exec(rawBody)) !== null) {
    ...
    exercises.push({
      ...
      type: compType === 'aeterna-formula' ? 'AeternaFormula' : compType === 'prediction-box' ? 'PredictionBox' : compType === 'parameter-lab' ? 'ParameterLab' : compType === 'graph-lab' ? 'GraphLab' : compType === 'error-hunter' ? 'ErrorHunter' : compType === 'model-builder' ? 'ModelBuilder' : compType === 'concept-map' ? 'ConceptMap' : compType === 'sequence-builder' ? 'SequenceBuilder' : compType,
      layerId,
      title,
      hasHint: false,
      hasXP: true,
  ...
  // 3. Extract <AeternaDecisionBox ... /> components
  const decisionBoxRegex = /<AeternaDecisionBox[\s\S]*?\/>/g;

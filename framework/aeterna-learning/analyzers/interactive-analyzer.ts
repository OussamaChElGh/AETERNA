...
export function analyzeInteractives(rawBody: string): {
  ...
  // 2. Legacy JSX components: <AeternaDecisionBox>, <Interactive*>, etc.
  const interactiveRegex = /<(AeternaDecisionBox|Interactive[A-Za-z0-9_]+|NexusNode3D|ProgresionArticulo|Simulacion[A-Za-z0-9_]+)/g;
...
function evaluateInteractiveCriteria(compType: string, bodyText: string): InteractiveValueResult {
  ...
  if (compType === 'AeternaDecisionBox' || compType === 'aeterna-decision') {

...
export function analyzeKnowledgeBenchmark(
  ...
  // 5. ANEKTIA ADDED VALUE EVALUATION
  const addedValueReasons: string[] = [];
  let interactiveScoreBonus = 0;

  if (parsed.interactives.length > 0) {
    interactiveScoreBonus += 20;
    addedValueReasons.push(`Anektia proporciona ${parsed.interactives.length} componentes interactivos manipulables no disponibles en textos de referencia planos.`);
  }

  if (parsed.exercises.some(e => e.type === 'AnektiaDecisionBox' || e.type === 'PredictionBox' || e.type === 'ParameterLab' || e.type === 'ErrorHunter')) {
    interactiveScoreBonus += 20;
    addedValueReasons.push('Anektia exige predicción activa, toma de decisiones y análisis de errores en lugar de lectura pasiva.');
  }

...
const LEGACY_BLOCK_RE = /```(aeterna-question|anektia-equation)\b/g;
...
function inspectJsonArticle(slug: string): { sections: number; allLayers: boolean; legacyBlocks: number; blockTypesUsed: number; totalBlocks: number; imagesSuggested: number } {
  ...
  try {
    ...
    const legacyRe = /```(aeterna-question|anektia-equation)\b/g;
...
function classifyGap(
  ...
  if (inventory.jsonLegacyBlocks > 0) {
    return {
      ...
      orden: planned.orden,
      severity: 'MEDIA',
      reason: `El JSON usa ${inventory.jsonLegacyBlocks} bloques legacy no soportados (aeterna-question/anektia-equation)`,

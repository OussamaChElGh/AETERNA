# ADR 0001: Aeterna Learning Framework v1.0 (Content Auditor)

- **Estado**: Aprobado
- **Fecha**: 2026-07-30
- **Autores**: Equipo de Arquitectura Pedagógica Aeterna

---

## 1. Contexto

Aeterna publica contenido educativo estructurado en Markdown/MDX organizado en tres capas de profundización (*Inicio / Fundamentos*, *Intermedio / Profundización*, *Avanzado / Frontera*) acompañado de ejercicios (`AeternaExercise`, `AeternaDecisionBox`) y componentes interactivos.
...
Se implementa **Aeterna Learning Framework v1.0** como un módulo interno independiente ubicado en `/framework/aeterna-learning/` con CLI en `/scripts/audit-content.ts`.
...
2. **Reutilización de Infraestructura Existente**:
   ...
   - Parseo estructural robusto de bloques MDX (`<NivelActivo>`, `<IndiceNivel>`, ````aeterna-exercise`, `<AeternaDecisionBox>`, `<BotonTransicion>`).
...
/framework/aeterna-learning/
  ...
  │   ├── exercise-analyzer.ts       # Conteo y extracción de AeternaExercise / AeternaDecisionBox
...
- **Estructura de Componentes JSX/MDX**: Reconoce la sintaxis nativa utilizada en las guías de física (`<NivelActivo id="...">`, `<AeternaDecisionBox ...>`, ````aeterna-exercise`, `<BotonTransicion>`).

...
for (const [id, asset] of Object.entries(ROOM_ASSETS)) {
  ...
  for (const src of sources) {
    ...
    if (!fs.existsSync(fullPath)) {
      console.log(`❌ Falta en ROOM_ASSETS (${id}): ${src}`);
...
for (const item of ROOM_ITEM_CATALOG) {
  if (item.asset.type === 'pixel_art' && item.asset.src) {
    ...
    if (!fs.existsSync(fullPath)) {
      console.log(`❌ Falta en ROOM_ITEM_CATALOG (${item.id}): ${item.asset.src}`);
      missingCatalog.push(item.asset.src);
    }
  }
}

console.log(`\nResumen: ${missingEngine.length} activos faltantes en roomEngineAssets, ${missingCatalog.length} activos faltantes en roomCatalog.`);

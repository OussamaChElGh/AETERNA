...
const availableSubcategories = ["Todas", ...Array.from(new Set(selectionItems.filter(item => item.subcategory).map(item => item.subcategory)))];
    ...
    <div className="flex flex-col gap-32 pt-32">
      {!showsSelection && hydratedSteps.length === 0 && (
        <div className="mx-auto max-w-4xl w-full text-center py-24">
          ...
          >
            <div className="w-20 h-20 rounded-2xl bg-brand-gold/10 border-2 border-dashed border-brand-gold/40 flex items-center justify-center text-brand-gold">
              <Lock size={32} />
            </div>
            <h2 className="font-serif text-3xl md:text-5xl text-brand-offwhite">
              Esta rama está siendo desarrollada
            </h2>
            <p className="text-lg text-white/40 font-sans font-light max-w-xl mx-auto">
            ...
            <Link href="/guias/ciencias_naturales/fisica"
              className="mt-4 px-8 py-4 bg-brand-gold text-brand-ink text-[11px] font-sans font-bold uppercase tracking-[0.4em] hover:bg-brand-offwhite dark:hover:bg-[#1E1E22] hover:text-brand-gold transition-all shadow-[0_0_30px_rgba(212,175,55,0.3)]"
            >
              Explorar Física (disponible)
            </Link>
          </motion.div>
        </div>
      )}
        ...
        {!showsSelection && roadmap.content && (
          <div className="flex justify-center mb-24">
            <button 
              ...
              className="px-10 py-5 bg-brand-gold text-brand-ink text-[11px] font-sans font-bold uppercase tracking-[0.4em] hover:bg-brand-offwhite dark:hover:bg-[#1E1E22] hover:text-brand-gold transition-all shadow-[0_0_30px_rgba(212,175,55,0.2)] flex items-center gap-3"
            >
        ...
        <div className="flex flex-col gap-24">
          {levels.map((group, groupIndex) => (
            <div key={group.level?.num || groupIndex} className="relative">
              {group.level && (
                <div className="sticky top-24 z-20 bg-brand-offwhite/90 dark:bg-[#09090B]/90 backdrop-blur-md py-6 mb-12 border-b border-brand-border dark:border-white/5 relative">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6">
                      ...
                      <h2 className="font-serif text-2xl md:text-3xl text-brand-ink dark:text-brand-offwhite">
                        {group.level.title}
                      </h2>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-brand-ink dark:text-brand-offwhite px-4 py-2 bg-white dark:bg-[#1E1E22] border border-brand-border dark:border-white/5 shadow-sm">
                        {group.level.badge}
                      </span>
                    </div>
                  </div>
                  ...
                  >
                    <div className="flex flex-col md:flex-row gap-12 items-stretch">
                      ...
                      <div className={cn(
                        ...
                        step.type === "practice" ? "bg-slate-900 border-slate-800 ml-0 md:ml-12 mt-4 md:mt-0 shadow-2xl" : "bg-white dark:bg-[#121214] dark:border-white/5 hover:shadow-xl",
                        step.status === "available" && "border-brand-border opacity-90",
                        step.status === "current" && (step.type === "practice" ? "border-brand-gold/50 shadow-[0_10px_40px_rgba(212,175,55,0.15)]" : "border-brand-ink dark:border-brand-gold shadow-lg"),
                        ...
                        {(step.status === "current" || step.status === "completed" || step.status === "available") && (
                          <div className={cn("flex flex-wrap gap-6 pt-8 border-t relative z-10", step.type === "practice" ? "border-slate-800" : "border-brand-border")}>
                            {step.path ? (
                              <Link href={step.path} className={cn("px-8 py-4 text-[10px] font-sans font-bold uppercase tracking-[0.4em] transition-all", step.type === "practice" ? "bg-brand-gold text-slate-900 hover:bg-brand-offwhite" : "bg-brand-ink text-brand-offwhite hover:bg-brand-gold")}>
  ...
  return (
    <div className="min-h-screen bg-brand-offwhite dark:bg-[#09090B]">
      {/* Header Panel */}
          ...
          <div className="max-w-5xl mx-auto">
            ...
            >
              ...
              {!showsSelection && roadmap.content && (
                <button 
                  onClick={() => document.getElementById('ensayo-maestro')?.scrollIntoView({ behavior: 'smooth' })}
                  className="px-10 py-5 bg-brand-gold text-brand-ink text-[11px] font-sans font-bold uppercase tracking-[0.4em] hover:bg-brand-offwhite dark:hover:bg-[#1E1E22] hover:text-brand-gold transition-all shadow-[0_0_30px_rgba(212,175,55,0.4)]"
                >
      ...
      {roadmap.content && (
        <section id="ensayo-maestro" className="bg-white dark:bg-[#121214] border-b border-brand-border dark:border-white/5 py-40 px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-px bg-brand-border border border-brand-border">
              {selectionItems.map((item, i) => (
                <Link key={item.id}
                  ...
                  className="group relative bg-white dark:bg-[#121214] p-12 h-[360px] flex flex-col justify-between items-start transition-all hover:bg-brand-ink dark:hover:bg-[#1E1E22]"
                >
      ...
      {activeKey === "japones" && !showsSelection && (
          <div className="mt-16 space-y-16">
            <div className="bg-white dark:bg-[#121214] border sm:border-x-0 border-brand-border dark:border-white/5 py-24 px-8 md:px-0">

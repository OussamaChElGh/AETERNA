...
const LEVEL_PALETTE = {
  gold: {
    ...
    done:        'bg-[#1C1510] border-[#D4AF37] shadow-[0_0_0_6px_rgba(212,175,55,0.2)]',
    active:      'bg-[#2A1E08] border-[#EFD75F] shadow-[0_0_0_8px_rgba(239,215,95,0.25),0_0_20px_rgba(239,215,95,0.35)]',
    ...
    chestGlow:   'hover:shadow-[0_0_16px_rgba(212,175,55,0.4)]',
  },
  purple: {
    band:        'border-[#2A2045]',
    ...
    done:        'bg-[#1C1A28] border-[#A78BFA] shadow-[0_0_0_6px_rgba(167,139,250,0.2)]',
    active:      'bg-[#22203A] border-[#C4B5FD] shadow-[0_0_0_8px_rgba(196,181,253,0.25),0_0_20px_rgba(196,181,253,0.35)]',
    ...
    chestGlow:   'hover:shadow-[0_0_16px_rgba(167,139,250,0.4)]',
  },
  teal: {
    band:        'border-[#0A2020]',
    ...
    done:        'bg-[#0A1E18] border-[#2DD4BF] shadow-[0_0_0_6px_rgba(45,212,191,0.2)]',
    active:      'bg-[#0D2820] border-[#5EEAD4] shadow-[0_0_0_8px_rgba(94,234,212,0.25),0_0_20px_rgba(94,234,212,0.35)]',
    ...
    chestGlow:   'hover:shadow-[0_0_16px_rgba(45,212,191,0.4)]',
  },
  red: {
    band:        'border-[#2A1010]',
    ...
    done:        'bg-[#1E0F0F] border-[#F87171] shadow-[0_0_0_6px_rgba(248,113,113,0.2)]',
    active:      'bg-[#2A1010] border-[#FCA5A5] shadow-[0_0_0_8px_rgba(252,165,165,0.25),0_0_20px_rgba(252,165,165,0.35)]',
    ...
    chestGlow:   'hover:shadow-[0_0_16px_rgba(248,113,113,0.4)]',
  },
}
  ...
  return (
    <div className="flex gap-1 mt-1">
      {Array.from({ length: count }).map((_, i) => (
        <span
          key={i}
          className={cn('text-xs transition-opacity', i < lit ? 'opacity-100' : 'opacity-20')}
          style={{ color: '#D4AF37' }}
        >
          ★
        </span>
...
function Tooltip({ title, description }: { title: string; description: string }) {
  return (
    <div className={cn(
      ...
      'w-[180px] rounded-xl p-3 z-20',
      'bg-[#1C1810] border border-[#D4AF37] border-opacity-60',
      'opacity-0 group-hover:opacity-100 transition-opacity duration-150',
      'pointer-events-none text-left'
    )}>
      <p className="text-xs font-semibold text-[#D4AF37] mb-1 leading-tight">{title}</p>
      <p className="text-[11px] text-[#8B6914] leading-relaxed">{description}</p>
    </div>
  )
}
...
function NodeCircle({
  ...
  const circleClass = cn(
    'relative w-[88px] h-[88px] rounded-full flex items-center justify-center text-3xl',
  ...
  return (
    ...
    >
      ...
      {node.status === 'done' && (
        <span className={cn(
          'absolute -top-1.5 -right-1.5 w-[22px] h-[22px] rounded-full',
...
function RouteNodeComponent({
  ...
  const labelClass = cn(
    'text-xs font-semibold text-center max-w-[90px] leading-snug',
    node.status === 'done'   && palette.labelDone,
    node.status === 'active' && cn(palette.labelActive, 'font-bold'),
...
function VerticalConnector({ palette, locked = false }: { palette: typeof LEVEL_PALETTE['gold']; locked?: boolean }) {
  return (
    <div className={cn(
      'w-[3px] h-10 bg-gradient-to-b flex-shrink-0 rounded-full',
  ...
  return (
    <div className="relative">
      <button
        ...
        disabled={!unlocked}
        className={cn(
          'relative group w-20 h-14 rounded-xl flex flex-col items-center justify-center gap-1',
      ...
      >
        ...
        <span className="text-[9px] font-bold tracking-widest uppercase" style={{ color: unlocked ? '#D4AF37' : '#3D3A30' }}>
          {unlocked ? 'Abrir' : 'Sellado'}
        </span>

        {unlocked && (
          <span className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-[#D4AF37] text-[#16140F] text-[10px] font-bold flex items-center justify-center animate-bounce">
            !
          </span>
        )}
      </button>

      {/* Reward preview globe */}
      {showPreview && unlocked && reward && (
        <div className={cn(
          'absolute left-1/2 -translate-x-1/2 bottom-full mb-3',
          'w-[220px] rounded-2xl p-4 z-30',
          'bg-[#1C1810] border border-[#D4AF37] border-opacity-70',
          'shadow-[0_8px_32px_rgba(0,0,0,0.6),0_0_24px_rgba(212,175,55,0.2)]',
          'animate-in zoom-in-95 fade-in duration-200'
        )}>
          <div className="flex items-start gap-3">
            <div className="w-12 h-12 rounded-xl bg-[#2A2415] border border-[#D4AF37]/30 flex items-center justify-center shrink-0 overflow-hidden">
              <img src={reward.image} alt={reward.name} className="w-10 h-10 object-contain" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#8B6914] mb-0.5">Recompensa</p>
              <p className="text-xs font-bold text-[#D4AF37] leading-tight">{reward.name}</p>
              <p className="text-[10px] text-[#6B5B35] mt-1 leading-relaxed">{reward.description}</p>
            </div>
            <button
              onClick={(e) => { e.stopPropagation(); setShowPreview(false) }}
              className="text-[#8B6914] hover:text-[#D4AF37] text-sm shrink-0"
            >✕</button>
          </div>
        </div>
      )}
...
function LevelBand({
  ...
  return (
    <div className={cn(
      'relative rounded-2xl border p-5 pb-6',
      'bg-[#16140F]',
      palette.band
    )}>
      <div className={cn(
        'absolute -top-[14px] left-1/2 -translate-x-1/2',
        'px-4 py-1 rounded-full border text-[11px] font-bold tracking-[0.2em]',
        'whitespace-nowrap',
        palette.title
      )}>
      ...
      <div className="flex justify-center mt-4">
        <span className="text-[10px] font-mono tracking-[0.15em] opacity-30" style={{ color: palette.labelActive }}>
...
function NodeModal({
  ...
  return (
    ...
    >
      <div
        className="bg-[#1C1810] border border-[#D4AF37] border-opacity-60 rounded-2xl p-7 max-w-sm w-full"
      ...
      >
        <div className="flex items-start justify-between mb-5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[11px] text-[#8B6914] tracking-[0.2em] font-bold mb-1">
                PARADA {node.order}
              </p>
              <h3 className="text-[#D4AF37] font-bold text-xl leading-tight">{node.title}</h3>
            </div>
          </div>
          <button
            onClick={onClose}
            className="text-[#8B6914] hover:text-[#D4AF37] transition-colors text-xl leading-none"
            aria-label="Cerrar"
          >
            ✕
          </button>
        ...
        <p className="text-[#8B7720] text-sm leading-relaxed mb-6">{node.description}</p>

        <div className="flex items-center justify-between mb-6">
          <div className="flex gap-5">
            <div className="text-center">
              <p className="text-[#D4AF37] font-bold text-xl">{node.xp}</p>
              <p className="text-[#8B6914] text-[11px]">XP máx.</p>
            </div>
            <div className="text-center">
              <p className="text-[#D4AF37] font-bold text-xl">
                {node.type === 'theory' ? '📖' : '⚡'}
              </p>
              <p className="text-[#8B6914] text-[11px]">
        ...
        {node.status === 'locked' ? (
          <div className="text-center py-4 rounded-xl bg-[#16140F] border border-[#2A2415]">
            <p className="text-[#555449] text-sm">Completa la parada anterior para desbloquear</p>
          </div>
        ) : (
          <button
            ...
            className={cn(
              ...
              node.status === 'done'
                ...
                : 'bg-[#D4AF37] text-[#16140F] shadow-[0_4px_20px_rgba(212,175,55,0.3)]'
            )}
          >
...
export function RouteMap({ route, userProgress, onNodeStart }: RouteMapProps) {
  ...
  return (
    <>
      <div className="w-full max-w-xl mx-auto px-4 py-4 flex flex-col gap-4">
        ...
        {route.levels.map((level, levelIdx) => (
          <div key={level.id} className="flex flex-col items-center gap-0">
            ...
            {levelIdx < route.levels.length - 1 && (
              <div className={cn(
                'w-[3px] h-10 bg-gradient-to-b opacity-30 rounded-full',
        ...
        <div className={cn(
          'flex items-center justify-between mt-3 px-5 py-4 rounded-2xl',
          'bg-[#16140F] border border-[#2A2415]'
        )}>
          <div>
            <p className="text-[11px] text-[#8B6914] tracking-[0.2em] font-bold mb-1 uppercase">
          ...
          <div className="text-right">
            <p className="text-[11px] text-[#8B6914] mb-1.5">Progreso global</p>
            <div className="w-36 h-2 bg-[#2A2415] rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-[#B8860B] to-[#D4AF37] rounded-full transition-all duration-700"
                style={{
                  width: `${Math.min(100, totalNodes > 0 ? (userProgress.completedNodes.length / totalNodes) * 100 : 0)}%`
                }}
              />
            </div>
            <p className="text-[11px] text-[#8B6914] mt-1.5">
              {userProgress.completedNodes.length} / {totalNodes} paradas
            </p>
          </div>
        </div>
      </div>
      ...
      {openedChest && (
        <div className={cn(
          ...
          <div>
            <p className="text-[#D4AF37] font-bold text-sm">¡Cofre desbloqueado!</p>

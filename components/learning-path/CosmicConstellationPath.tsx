...
import { ArtifactStatsPanel } from './ArtifactStatsPanel';
...
export default function CosmicConstellationPath() {
  ...
  const totalLayers = nodes.length * 3;
  const relicsCount = progress.physicsRelics?.length || 0;
  const totalRelics = 4;
  const xpForLevel = 1200;
  const xpProgress = Math.min((userXp % xpForLevel) / xpForLevel, 1);
  ...
  return (
    <div className="min-h-screen font-sans" style={{ background: CHARCOAL, color: CREAM }}>
      ...
      <div className="max-w-6xl mx-auto px-6 py-10 relative z-10 flex gap-12">
        {/* LEFT: Path */}
              ...
              return (
                <div key={lvl.nivel}>
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <span className="text-[9px] font-mono font-black uppercase tracking-[0.3em]" style={{ color: 'rgba(255,255,255,0.3)' }}>Nivel {lvl.nivel}</span>
                    <span className="text-[8px] font-mono ml-auto" style={{ color: 'rgba(255,255,255,0.15)' }}>{lvlDone}/{lvl.nodes.length}</span>
                  </div>
                  <div className="h-[2px] rounded-full mb-5" style={{ background: 'rgba(255,255,255,0.05)' }}>
                    <motion.div className="h-full rounded-full transition-all duration-700" style={{ width: `${progPct}%`, background: progPct >= 100 ? GOLD : accent }} />
                  </div>
                  <div className="space-y-2">
                    {lvl.nodes.map((node, ni) => (
                      <Link key={node.slug} href={node.unlocked ? `/guias/ciencias_naturales/fisica/${node.slug}` : '#'} className={cn("block group", !node.unlocked && "pointer-events-none")}>
                        <motion.div
                          ...
                          style={{ background: node.unlocked ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.005)', opacity: node.unlocked ? 1 : 0.3 }}
                        >
                          <div className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 border-2 transition-all duration-300"
                            style={{ borderColor: node.completed ? GOLD : node.inProgress ? '#34D399' : node.unlocked ? accent : 'rgba(255,255,255,0.08)', background: node.completed ? `radial-gradient(circle, ${GOLD}20, transparent)` : 'transparent', boxShadow: node.completed ? `0 0 12px ${GOLD}30` : 'none' }}>
                            {node.completed ? <CheckCircle size={18} style={{ color: GOLD }} /> : node.unlocked ? <Circle size={18} style={{ color: accent+'80' }} /> : <Lock size={14} style={{ color: 'rgba(255,255,255,0.1)' }} />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <h3 className="font-serif text-sm font-bold leading-tight" style={{ color: node.unlocked ? CREAM : 'rgba(229,229,229,0.2)' }}>{node.title}</h3>
                            {node.unlocked && (
                              <div className="flex items-center gap-2 mt-1">
                                <div className="flex gap-0.5">
                                  {[0, 1, 2].map(i => (<div key={i} className="w-1.5 h-1.5 rounded-full" style={{ background: i < node.layers ? GOLD : 'rgba(255,255,255,0.06)' }} />))}
                                </div>
                                <span className="text-[9px] font-mono" style={{ color: 'rgba(229,229,229,0.25)' }}>{node.completed ? 'Completado' : node.inProgress ? `${node.layers}/3 capas` : '3 capas'}</span>
                              </div>
                            )}
                          </div>
                          {node.unlocked && (
                            <motion.div animate={{ x: [0, 3, 0] }} transition={{ duration: 2, repeat: Infinity }} style={{ color: 'rgba(197,160,89,0.3)' }}>
        ...
        {/* RIGHT: Artifact Stats Panel */}
        <div className="w-[340px] shrink-0 hidden lg:block pt-20">
          <ArtifactStatsPanel
            xp={userXp}
            level={userLevel}
            dailyStreak={dailyStreak}
            relicsCount={relicsCount}
            totalRelics={totalRelics}
            layersDone={totalLayersDone}
            totalLayers={totalLayers}
            emblemsDone={doneCount}
            totalEmblems={nodes.length}
